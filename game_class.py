...
class MancalaGame:
    def __init__(self):
        ...
        self.board._cell_color = "white"
        self.board.create_output(background_color="black", color="white", font_size=12)
    ...
    def initialise_board_ui(self):
        ...
        for row in range(1, 3):
            for col in range(1, 7):
                self.board[row][col] = RockStack().stack_images(4, row, col)

            # Set player's cups to 0
        self.board[1][0] = "Bot"
        self.board[2][0] = RockStack().stack_images(4, 2, 0)
        self.board[1][7] = RockStack().stack_images(4, 1, 7)
        self.board[2][7] = self.username
    ...
    def mouse_click(self, btn: int, row: int, col: int) -> None:
        print(f"Row: {row}")
        print(f"Col: {col}")
        print(f"Value: {self.board_dictionary[f'Row_{row}'][col-2]}")
        ...
        if self.board_dictionary[f"Row_{row}"][col-1] == 0 or row != 2 or col in {0, 7} or row in {0, 3}:
    ...
    def board_update(self, board_dictionary: dict) -> None:
        ...
        output: None"""
        for col in range(1, 7):
            self.board[1][COLUMNS - col - 1] = RockStack().stack_images(board_dictionary["Row_1"][col - 1], 1, COLUMNS-col-1)
            self.board[2][col] = RockStack().stack_images(board_dictionary["Row_2"][col - 1], 2, col)
        self.board[1][7] = RockStack().stack_images(board_dictionary["Player2_score"], 1, 7)
        self.board[2][0] = RockStack().stack_images(board_dictionary["Player1_score"], 2, 0)
