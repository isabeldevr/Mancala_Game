...
class MancalaGame:
    ...
    def board_update(self, board_dictionary: dict) -> None:
     ...
     def ai_player(self) -> (int, int):
        """This method is the AI player using MCTS"""
        root_state = copy.deepcopy(self.board_dictionary)
        root = MCTSNode(root_state)

        iterations = 1000  # Adjust this value as needed for the number of iterations in MCTS

        for _ in range(iterations):
            node = root
            while node.children:
                node = select_child(node)
            if node.visits == 0 and not self.check_game_over():
                expand(node)
        ...
        move = self.get_move_from_states(root_state, best_state)
        return move
    def get_move_from_states(self, state_before, state_after) -> (int, int):
        """Get the move based on the states before and after the MCTS simulation"""
        for row in range(1, 3):
            for col in range(6):
                if state_before[f"Row_{row}"][col] != state_after[f"Row_{row}"][col]:
                    return row, col
        return None
